...
public class ArtifactDao extends Dao {
    public List<Artifact> getArtifacts() {
        ...
        connect();
        try {
            ResultSet results = statement.executeQuery("SELECT * FROM Artifacts");
            while (results.next()) {
    ...
    public Artifact createArtifact(ResultSet results) throws SQLException {
        ...
        return new Artifact(artifact_ID, artifact_Name, cost, is_Active, description, is_Solo, collected);
    }

    public void addNewArtifact(Artifact artifact) {
        connect();
        PreparedStatement addNewArtifact;
        String sql = "INSER INTO Artifacts (Artifact_Name, Cost, Is_Active, Description, Is_Solo, Collected) VALUES (?, ?, ?, ?, ?, ?)";
        try {
